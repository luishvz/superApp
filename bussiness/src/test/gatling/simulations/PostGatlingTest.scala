...
class PostGatlingTest extends Simulation {
    ...
    val authorization_header = "Basic " + Base64.getEncoder.encodeToString("bussinessapp:my-secret-token-to-change-in-production".getBytes(StandardCharsets.UTF_8))
    ...
    val scn = scenario("Test the Post entity")
        ...
        .formParam("client_id", "bussinessapp")
        ...
        .repeat(2) {
            ...
            .get("/bussiness/api/posts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new post")
            .post("/bussiness/api/posts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "title":"SAMPLE_TEXT", "content":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_post_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created post")
                .get("/bussiness${new_post_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created post")
            .delete("/bussiness${new_post_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
